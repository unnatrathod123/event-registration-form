...
export default function EventRegisterPage() {
  ...
  const [isFetching, setIsFetching] = useState(true); // Added state to track API loading
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      setIsFetching(true); // Ensure fetching state is true when starting
      try {
        ...
        setIsFetching(false); // Stop loading indicator regardless of success or error
      }
    };
    fetchEvents();
  }, []);
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-900 py-12 px-4 relative overflow-hidden">
      ...
      {alert.show && (
        <div className={`fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center p-4 w-full max-w-xs rounded-xl shadow-2xl border ${alert.type === "success" ? "bg-green-600 border-green-700" : "bg-red-600 border-red-700"} text-white`}>
          ...
          <button onClick={() => setAlert(prev => ({ ...prev, show: false }))} className="ml-auto bg-white/20 p-1 rounded-md hover:bg-white/30 transition">✕</button>
        </div>
      )}

      {/* Increased max-width for larger 4K screens */}
      <div className="relative z-10 w-full max-w-4xl xl:max-w-6xl space-y-8">
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border border-slate-100">
          ...
          <div className="flex flex-col md:flex-row md:justify-between items-center md:items-start gap-6 mb-8 text-center md:text-left">
            <Image 
              ...
              height={100}
              priority 
              className="w-48 md:w-56 lg:w-64 h-auto object-contain md:order-2 shrink-0" 
            />
            <div className="md:order-1 flex-1">
              <h1 className="text-2xl lg:text-3xl font-extrabold text-slate-900">Active Events</h1>
              <p className="text-slate-500 text-sm lg:text-base mt-1">Select the events you wish to participate in</p>
            </div>
          </div>

          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {/* Updated render logic to handle fetching, empty state, and populated state */}
            {isFetching ? (
              ...
              events.map(ev => {
                ...
                return (
                  <div 
                    ...
                    className={`group cursor-pointer p-5 rounded-2xl border-2 transition-all duration-200 flex flex-col h-full ${
                      isSelected 
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 line-clamp-2 italic leading-relaxed mt-auto pt-2 border-t border-slate-200/50">
                      {ev.description}
                    </p>
                  </div>
                );
              ...
              <div className="col-span-full py-16 px-4 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
                <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4">
                  <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                </div>
                <h3 className="text-lg font-bold text-slate-700 mb-1">No Active Events</h3>
                <p className="text-slate-500 text-sm text-center">There are no events currently open for registration.<br/>Please check back later.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* --- REGISTRATION FORM --- */}
        <form onSubmit={handleSubmit} className="bg-white/90 backdrop-blur-lg p-8 md:p-10 lg:p-12 rounded-3xl shadow-2xl border border-white/50">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-xl lg:text-2xl font-bold text-slate-800">Your Information</h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <Input label="Mobile Number (Preferably WhatsApp)" type="tel" placeholder="10-digit number" required value={form.phone} onChange={(v: string) => setForm({ ...form, phone: v.replace(/\D/g, "").slice(0, 10) })} />
          ...
          <button
            disabled={loading || events.length === 0 || form.event_ids.length === 0}
            type="submit"
            className={`mt-10 w-full py-4 lg:py-5 rounded-2xl font-bold text-white text-lg lg:text-xl transition-all transform active:scale-[0.98] ${
              loading || events.length === 0 || form.event_ids.length === 0 
...
function Input({ label, type = "text", placeholder, value, onChange, required }: any) {
  return (
    <div className="flex flex-col">
      <label className="block text-xs lg:text-sm font-bold mb-2 text-slate-500 uppercase tracking-wide">
        {label} {required && <span className="text-red-500">*</span>}
      </label>
      <input
        type={type} value={value} required={required} placeholder={placeholder}
        className="w-full border border-slate-200 p-4 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-slate-50/50 text-slate-900 transition-all shadow-sm text-sm lg:text-base"
